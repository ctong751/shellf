...
const HOME_SAVED_LIMIT = 24
const HOME_WATCHING_LIMIT = 12
...
export const loadHomeMedia = async (authorDid: string): Promise<HomeMedia> => {
  const {
    ...
    const parents =
      parentIds.length > 0
        ? await db.select().from(content).where(inArray(content.id, parentIds))
        : []
    ...
    return {
      ...
      savedRecords: saved.slice(0, HOME_SAVED_LIMIT),
      watchingCount: uniqueStarts.length,
      watchingRecords: uniqueStarts
        .slice(0, HOME_WATCHING_LIMIT)
        .map((record) => ({
          ...record,
